      ...
      <div style={styles.modal}>
        ...
        {callType === 'video' && callStatus === 'connected' && (
          <div style={styles.videoContainer}>
            ...
            <video ref={myVideo} autoPlay playsInline muted style={styles.myVideo} />
          </div>
        )}
        ...
        {(callType === 'audio' || callStatus !== 'connected') && (
          <div style={styles.audioContainer}>
            <div style={{ ...styles.callerAvatar, background: getColor(displayName) }}>
              {displayName?.slice(0, 2).toUpperCase()}
            </div>
            <h2 style={styles.callerName}>{displayName}</h2>
            <p style={styles.callStatus}>
              {callStatus === 'calling' && '📞 Вызов...'}
              {callStatus === 'incoming' && `📞 ${callType === 'video' ? 'Видео' : 'Аудио'} звонок`}
        ...
        <div style={styles.controls}>
          {callStatus === 'incoming' && (
            <>
              <button style={{ ...styles.btn, background: '#4caf50' }} onClick={answerCall}>
                📞
              </button>
              <button style={{ ...styles.btn, background: '#ff4444' }} onClick={rejectCall}>
                📵
              </button>
            </>
          )}
          ...
          {callStatus === 'connected' && (
            <>
              <button style={{ ...styles.btn, background: isMuted ? '#ff4444' : '#2a2a2a' }} onClick={toggleMute}>
                {isMuted ? '🔇' : '🎤'}
              </button>
              {callType === 'video' && (
                <button style={{ ...styles.btn, background: isCameraOff ? '#ff4444' : '#2a2a2a' }} onClick={toggleCamera}>
                  {isCameraOff ? '📵' : '📷'}
                </button>
              )}
              <button style={{ ...styles.btn, background: '#ff4444' }} onClick={endCall}>
                📵
              </button>
            </>
          )}
