...
export default function Chat() {
  ...
  const loadUsers = async () => {
    try {
      const res = await fetch(
        "https://gi-chat-production.up.railway.app/api/auth/users",
      );
      ...
      const blockRes = await fetch(
        `https://gi-chat-production.up.railway.app/api/auth/blocks/${user.username}`,
      );
  ...
  const showNotification = (from, text) => {
    if (Notification.permission === "granted") {
      if (document.hasFocus() && useStore.getState().currentChat === from)
        return;
      const notification = new Notification(`💬 GI Chat — ${from}`, {
        body: text?.startsWith(
          "https://gi-chat-production.up.railway.app/uploads/",
        )
          ? "🖼 Фото"
          : text,
        icon: "💬",
